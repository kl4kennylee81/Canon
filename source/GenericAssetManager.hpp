...
//  Version: 1/7/16
//
#ifndef GenericAssetManager_hpp
#define GenericAssetManager_hpp
...
private:
	...
	CU_DISALLOW_COPY_AND_ASSIGN(GenericAssetManager);

protected:
	/** The individual loaders for each type */
	std::unordered_map<size_t, std::shared_ptr<cugl::BaseLoader>> _handlers;
	/** The central thread for managing all of the loaders */
	std::shared_ptr<cugl::ThreadPool> _workers;

	/** State variable to manage reading JSON directories */
	bool _preload;

	std::shared_ptr<cugl::JsonValue> _jsonref;
	...
	*/
	bool readCategory(size_t hash, const std::shared_ptr<cugl::JsonValue>& json);
	...
	*/
	void readCategory(size_t hash, const std::shared_ptr<cugl::JsonValue>& json,
...
public:
	...
	*/
	GenericAssetManager() : _preload(false) {}

	/**
	* Deletes this asset manager, disposing of all resources.
	*/
	~GenericAssetManager() { dispose(); }
...
#pragma mark Static Constructors
	...
	*/
	static std::shared_ptr<GenericAssetManager> alloc() {
		std::shared_ptr<GenericAssetManager> result = std::make_shared<GenericAssetManager>();
	...
	*/
	static std::shared_ptr<GenericAssetManager> alloc(unsigned int threads) {
		std::shared_ptr<GenericAssetManager> result = std::make_shared<GenericAssetManager>();
...
#pragma mark Loader Management
	...
	*/
	template<typename T>
	bool attach(const std::shared_ptr<cugl::BaseLoader>& loader) {
	...
	*/
	template<typename T>
	std::shared_ptr<cugl::Loader<T>> access() {
		size_t hash = typeid(T).hash_code();
		auto it = _handlers.find(hash);
		if (it == _handlers.end()) {
		...
		return std::dynamic_pointer_cast<cugl::Loader<T>>(it->second);
	}
...
#pragma mark Loading/Unloading
	...
	*/
	...
	std::shared_ptr<T> get(const std::string& key) const {
		...
		std::shared_ptr<cugl::Loader<T>> loader = std::dynamic_pointer_cast<cugl::Loader<T>>(it->second);
	...
	*/
	template<typename T>
	void loadAsync(const std::string& key, const std::string& source, cugl::LoaderCallback callback) {
		size_t hash = typeid(T).hash_code();
		auto it = _handlers.find(hash);
		if (it != _handlers.end()) {
	...
	*/
	template<typename T>
	void loadAsync(const char* key, const std::string& source, cugl::LoaderCallback callback) {
	...
	*/
	template<typename T>
	void loadAsync(const std::string& key, const char* source, cugl::LoaderCallback callback) {
	...
	*/
	template<typename T>
	void loadAsync(const char* key, const char* source, cugl::LoaderCallback callback) {
...
#pragma mark Directory Support
	...
	*/
	virtual bool loadDirectory(const std::shared_ptr<cugl::JsonValue>& json);
	...
	* @param callback  An optional callback after each asset is loaded
	*/
	void loadDirectoryAsync(const std::shared_ptr<cugl::JsonValue>& json, cugl::LoaderCallback callback);
	...
	* @param callback  An optional callback after each asset is loaded
	*/
	void loadDirectoryAsync(const std::string& directory, cugl::LoaderCallback callback);
	...
	* @param callback  An optional callback after each asset is loaded
	*/
	void loadDirectoryAsync(const char* directory, cugl::LoaderCallback callback) {
